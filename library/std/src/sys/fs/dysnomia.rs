//! The dysnomia filesystem arm: a thin marshalling shell over the
//! storaged session client in the seam crate `dysnomia-sys`. Every real op is a
//! one-line delegation to a `__dysnomia_fs_*` `extern "Rust"` symbol (see
//! `sys/pal/dysnomia/mod.rs` for why the seam is a link-time symbol, not a direct
...
//! here and `read`/`write` pass an explicit offset. dysnomia's `OsStr` is bytes,
//! so a path crosses the seam as its raw encoded bytes; the seam splits
...
//!
//! Surface that is `Unsupported` by construction: symlinks
...
//! `duplicate`. Metadata carries the entry size and file/dir type —
...
//! lives in `dysnomia_sys::io_error` (its kinds surface through `decode_error_kind`).
...
/// Directory-aware metadata crossing the seam from `dysnomia_sys::fs::Meta`: 
/// the entry kind + size. `#[repr(C)]` with fields in the same order as the seam
...
/// One `read_dir` entry head crossing the seam from `dysnomia_sys::readdir::DirEntMeta`
/// `#[repr(C)]` with fields in the same order as the seam crate's
...
// Provided by the seam crate `dysnomia-sys` (its `#[no_mangle]` `pal.rs` shims over
// `dysnomia_sys::fs`). Raw path *bytes* cross the seam; a `< 0` return is a raw fs
...
unsafe extern "Rust" {
    fn __dysnomia_fs_read(path: &[u8], offset: u64, buf: &mut [u8]) -> i64;
    fn __dysnomia_fs_write(path: &[u8], offset: u64, data: &[u8]) -> i64;
    fn __dysnomia_fs_stat(path: &[u8]) -> i64;
    fn __dysnomia_fs_metadata(path: &[u8]) -> FsMeta;
    fn __dysnomia_fs_rename(from: &[u8], to: &[u8]) -> i64;
    fn __dysnomia_fs_unlink(path: &[u8]) -> i64;
    fn __dysnomia_fs_sync() -> i64;
    fn __dysnomia_fs_readdir_open(path: &[u8]) -> i64;
    fn __dysnomia_fs_readdir_next(handle: i64, name_buf: &mut [u8]) -> FsDirEntMeta;
    fn __dysnomia_fs_readdir_close(handle: i64);
}

/// The raw path bytes dysnomia sends over the seam (its `OsStr` is bytes).
...
/// The fs codes fit in `i32` by construction (`dysnomia_sys::io_error` band).
...
/// Minimal file attributes: the content size and whether the entry
...
impl FileAttr {
    ...
    pub fn modified(&self) -> io::Result<SystemTime> {
        // mtime is a mandatory field absent from the current wire protocol
...
impl Iterator for ReadDir {
    ...
    fn next(&mut self) -> Option<io::Result<DirEntry>> {
        // The seam bounds a name at the 255-byte path component, so a listing
        ...
        let head = unsafe { __dysnomia_fs_readdir_next(self.handle, &mut name) };
        match head.code {
            0 => Some(Ok(DirEntry {
...
impl Drop for ReadDir {
    fn drop(&mut self) {
        ...
        // from `__dysnomia_fs_readdir_open` and is closed exactly once (here).
        unsafe { __dysnomia_fs_readdir_close(self.handle) };
    }
}
...
fn stat_size(bytes: &[u8]) -> i64 {
    ...
    unsafe { __dysnomia_fs_stat(bytes) }
}
...
impl File {
    pub fn open(path: &Path, opts: &OpenOptions) -> io::Result<File> {
        ...
        if opts.truncate && !opts.create_new {
            ...
            let _ = unsafe { __dysnomia_fs_unlink(bytes) };
        }
    ...
    pub fn fsync(&self) -> io::Result<()> {
        // storaged syncs the whole ref, a superset of one file's fsync.
        // SAFETY: plain marshalling call.
        let r = unsafe { __dysnomia_fs_sync() };
    ...
    pub fn read(&self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let r = unsafe { __dysnomia_fs_read(path_bytes(&self.path), off, buf) };
        if r < 0 {
            return Err(err(r));
        }
        let n = r as usize;
    ...
    pub fn write(&self, buf: &[u8]) -> io::Result<usize> {
        let off = self.pos.load(Ordering::Relaxed);
        // SAFETY: plain marshalling call; `buf` outlives it.
        let r = unsafe { __dysnomia_fs_write(path_bytes(&self.path), off, buf) };
        if r < 0 {
            return Err(err(r));
        }
        let n = r as usize;
...
pub fn readdir(p: &Path) -> io::Result<ReadDir> {
    ...
    let handle = unsafe { __dysnomia_fs_readdir_open(path_bytes(p)) };
    if handle < 0 {
...
pub fn unlink(p: &Path) -> io::Result<()> {
    // SAFETY: plain marshalling call.
    let r = unsafe { __dysnomia_fs_unlink(path_bytes(p)) };
    if r == 0 { Ok(()) } else { Err(err(r)) }
}

pub fn rename(old: &Path, new: &Path) -> io::Result<()> {
    // SAFETY: plain marshalling call.
    let r = unsafe { __dysnomia_fs_rename(path_bytes(old), path_bytes(new)) };
    if r == 0 { Ok(()) } else { Err(err(r)) }
}
...
/// Directory-aware metadata: the seam probes `Stat` then `List`, so a
...
fn stat_attr(p: &Path) -> io::Result<FileAttr> {
    ...
    let m = unsafe { __dysnomia_fs_metadata(path_bytes(p)) };
    if m.code < 0 {
