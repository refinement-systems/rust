...
// Provided by the seam crate `eunomia-sys` (see `sys/pal/eunomia/mod.rs` for why these
...
unsafe extern "Rust" {
    fn __eunomia_mono_ns() -> i64;
    fn __eunomia_wall_ns() -> i64;
}
...
impl Instant {
    pub fn now() -> Instant {
        // `__eunomia_mono_ns` is monotone and non-negative by construction (urt's
        ...
        let ns = unsafe { __eunomia_mono_ns() };
