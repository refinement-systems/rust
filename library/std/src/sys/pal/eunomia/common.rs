...
pub fn abort_internal() -> ! {
    // std owns the panic handler in a std binary, so the reaper contract is
    ...
    // `dysnomia_sys::syscall::STATUS_PANIC` (`u64::MAX`), duplicated here because std
    // cannot depend on the seam crate — the same posture as the `ERR_*` discriminants in
    // `sys/io/error/dysnomia.rs`; it stays in lockstep with that crate.
    unsafe extern "Rust" {
        fn __dysnomia_thread_exit(code: u64) -> !;
    }
    unsafe { __dysnomia_thread_exit(u64::MAX) }
}
