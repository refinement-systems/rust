//! Futex over the dysnomia seam: `sys::futex`, the one primitive that
//! backs the whole upstream lock stack (`Mutex`/`Condvar`/`RwLock`/`Once`/`Parker`).
//!
//! Delegates to the seam crate `dysnomia-sys` through `extern "Rust"` symbols (see
//! `sys/pal/dysnomia/mod.rs` for why these are undefined externs and not a direct
...
// Provided by the seam crate `dysnomia-sys` (the `__rust_alloc` pattern): the
// `urt::futex` notif-backed table. `timeout_ns == u64::MAX` means "no timeout".
unsafe extern "Rust" {
    fn __dysnomia_futex_wait(futex: &Atomic<u32>, expected: u32, timeout_ns: u64) -> bool;
    fn __dysnomia_futex_wake(futex: &Atomic<u32>) -> bool;
    fn __dysnomia_futex_wake_all(futex: &Atomic<u32>);
}
...
pub fn futex_wait(futex: &Atomic<u32>, expected: u32, timeout: Option<Duration>) -> bool {
    ...
    unsafe { __dysnomia_futex_wait(futex, expected, timeout_ns) }
}
...
pub fn futex_wake(futex: &Atomic<u32>) -> bool {
    unsafe { __dysnomia_futex_wake(futex) }
}

/// Wakes up all threads that are waiting on `futex_wait` on this futex.
pub fn futex_wake_all(futex: &Atomic<u32>) {
    unsafe { __dysnomia_futex_wake_all(futex) }
}
