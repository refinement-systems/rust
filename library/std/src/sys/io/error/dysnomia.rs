...
// Provided by the seam crate `dysnomia-sys` (see `sys/pal/dysnomia/mod.rs`): the
...
// static label. dysnomia is a microkernel with no ambient errno (errors are the negative
// `ERR_*` syscall return values) and no signals.
unsafe extern "Rust" {
    fn __dysnomia_io_classify(code: i64) -> u8;
    fn __dysnomia_io_message(code: i64) -> &'static str;
}
...
pub fn decode_error_kind(code: i32) -> ErrorKind {
    // Mirrors `dysnomia_sys::io_error::Kind` (the host-tested single source of truth);
    // its `#[repr(u8)]` discriminants are fixed in lockstep with this match. The 7..
    // discriminants are the decision table; each is a stable
    // `io::ErrorKind`. `5` (`Kind::Uncategorized`) rides the `_` fallback.
    match unsafe { __dysnomia_io_classify(code as i64) } {
...
pub fn error_string(errno: i32) -> String {
    unsafe { __dysnomia_io_message(errno as i64) }.to_string()
}
