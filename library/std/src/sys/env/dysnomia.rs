...
// Provided by the seam crate `dysnomia-sys` (see `sys/pal/dysnomia/mod.rs`): the stashed
// environment as raw `KEY=VALUE` byte-strings borrowed from the startup block.
unsafe extern "Rust" {
    fn __dysnomia_env() -> &'static [&'static [u8]];
}
...
pub fn getenv(key: &OsStr) -> Option<OsString> {
    ...
    unsafe { __dysnomia_env() }.iter().find_map(|entry| {
        let (k, v) = split_kv(entry);
        (k == want).then(|| to_os(v))
    })
}
...
// spawner, and there is no shared mutable environ to mutate, so these
