...
// thread primitive over `urt::thread`. `spawn` shares the process's
...
unsafe extern "Rust" {
    fn __dysnomia_thread_spawn(entry: usize, stack: usize, arg: u64) -> i64;
    fn __dysnomia_thread_join(handle: u64) -> i64;
    fn __dysnomia_thread_yield();
    fn __dysnomia_thread_sleep(nanos: u64);
    fn __dysnomia_thread_exit(code: u64) -> !;
    /// Set up this spawned thread's `TPIDR_EL0` TLS block before
    /// `ThreadInit::init` runs `set_current` (which needs per-thread storage).
    fn __dysnomia_tls_init_thread();
    /// Run this thread's `thread_local!` destructors at exit.
    fn __dysnomia_tls_run_dtors();
    /// Reclaim this thread's heap TLS block at exit.
    fn __dysnomia_tls_free_thread();
}
...
impl Thread {
    ...
    pub unsafe fn new(stack: usize, init: Box<ThreadInit>) -> io::Result<Thread> {
        ...
        extern "C" fn thread_start(arg: u64) -> ! {
            unsafe {
                ...
                __dysnomia_tls_free_thread();
                __dysnomia_thread_exit(0)
            }
        }

        let arg = Box::into_raw(init).expose_provenance() as u64;
        let h = unsafe { __dysnomia_thread_spawn(thread_start as usize, stack, arg) };
        if h < 0 {
    ...
    pub fn join(self) {
        let r = unsafe { __dysnomia_thread_join(self.handle) };
        assert!(r >= 0, "dysnomia thread join failed ({r})");
    }
}
...
pub fn yield_now() {
    unsafe { __dysnomia_thread_yield() }
}

pub fn sleep(dur: Duration) {
    let nanos = dur.as_nanos().min(u64::MAX as u128) as u64;
    unsafe { __dysnomia_thread_sleep(nanos) }
}
